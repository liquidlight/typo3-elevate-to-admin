...
class AdminElevationMiddlewareFunctionalTest extends FunctionalTestCase
{
	...
	public function testMiddlewareProcessesPermanentAdminUser(): void
	{
		...
		$this->assertEquals(1, $updatedRecord[DatabaseConstants::FIELD_IS_POSSIBLE_ADMIN]);
	}
	...
	public function testMiddlewareClearsExpiredElevation(): void
	{
		...
		$connection->update(
			...
			[
				'admin' => 1,
				DatabaseConstants::FIELD_ADMIN_SINCE => $expiredTime,
			],
			['uid' => $this->testUserId]
		);
	...
	public function testMiddlewareRefreshesValidElevation(): void
	{
		...
		$connection->update(
			...
			[
				'admin' => 1,
				DatabaseConstants::FIELD_ADMIN_SINCE => $recentTime,
			],
			['uid' => $this->testUserId]
		);
		...
		$this->assertGreaterThan($recentTime, $updatedRecord[DatabaseConstants::FIELD_ADMIN_SINCE]); // Timestamp updated
		$this->assertGreaterThanOrEqual($beforeProcessTime, $updatedRecord[DatabaseConstants::FIELD_ADMIN_SINCE]);
		$this->assertLessThanOrEqual($afterProcessTime, $updatedRecord[DatabaseConstants::FIELD_ADMIN_SINCE]);
		$this->assertEquals(1, $updatedRecord[DatabaseConstants::FIELD_IS_POSSIBLE_ADMIN]);
	}
	...
	public function testMiddlewareIgnoresNonAdminUsers(): void
	{
		...
		$originalRecord = $this->getDatabaseRecord(DatabaseConstants::TABLE_BE_USERS, [
			'uid' => $this->testUserId,
		]);
		...
		$this->assertEquals($originalRecord[DatabaseConstants::FIELD_ADMIN_SINCE], $updatedRecord[DatabaseConstants::FIELD_ADMIN_SINCE]);
		$this->assertEquals($originalRecord[DatabaseConstants::FIELD_IS_POSSIBLE_ADMIN], $updatedRecord[DatabaseConstants::FIELD_IS_POSSIBLE_ADMIN]);
	}
	...
	public function testMiddlewareElevationTimeout(): void
	{
		...
		$connection->update(
			...
			[
				'admin' => 1,
				DatabaseConstants::FIELD_ADMIN_SINCE => $exactTimeoutTime,
			],
			['uid' => $this->testUserId]
		);
		...
		$this->assertTrue(
			($updatedRecord['admin'] == 1 && $updatedRecord[DatabaseConstants::FIELD_ADMIN_SINCE] > $exactTimeoutTime) ||
			($updatedRecord['admin'] == 0 && $updatedRecord[DatabaseConstants::FIELD_ADMIN_SINCE] == 0)
		);
	}
